...
import ddata.custom.protobuf.messages
...
class TwoPhaseSetSerializer(val system: ExtendedActorSystem) extends Serializer with SerializationSupport {
  ...
  private def twoPhaseSetToProto(twoPhaseSet: TwoPhaseSet[_]): messages.TwoPhaseSet = {
    messages.TwoPhaseSet()
      .withAdds(ByteString.copyFrom(otherMessageToProto(twoPhaseSet.adds).toByteArray))
      .withRemovals(ByteString.copyFrom(otherMessageToProto(twoPhaseSet.removals).toByteArray))
  }

  private def twoPhaseSetFromBinary(bytes: Array[Byte]): TwoPhaseSet[_] ={
    val message = messages.TwoPhaseSet.parseFrom(bytes)
    val adds = otherMessageFromBinary(message.adds.toByteArray).asInstanceOf[GSet[Any]]
    val removals = otherMessageFromBinary(message.removals.toByteArray).asInstanceOf[GSet[Any]]
