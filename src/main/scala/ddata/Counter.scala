...
object Counter {
  ...
  private case class InternalUpdateResponse(response: Replicator.UpdateResponse[GCounter])
    extends InternalCommand
  private case class InternalGetResponse(response: Replicator.GetResponse[GCounter], replyTo: ActorRef[Int])
    extends InternalCommand
  private case class InternalSubscribeResponse(response: Replicator.SubscribeResponse[GCounter])
    extends InternalCommand
    ...
    Behaviors.setup { context =>
      ...
      DistributedData.withReplicatorMessageAdapter[Command, GCounter] { replicatorAdapter =>
        ...
        replicatorAdapter.askUpdate(
          Replicator.Update(key, GCounter.empty, Replicator.WriteLocal)(identity),
          InternalUpdateResponse
        )
        def updated(cachedValue: Int): Behavior[Command] = Behaviors.receiveMessage {
          case Increment =>
            replicatorAdapter.askUpdate(
          ...
          case internal: InternalCommand =>
            internal match {
              case InternalUpdateResponse(_) =>
                Behaviors.same
              case InternalGetResponse(response @ Replicator.GetSuccess(`key`), replyTo) =>
                replyTo ! response.get(key).value.toInt
                Behaviors.same
              case InternalGetResponse(_, _) =>
                // Not dealing with failures
                Behaviors.unhandled
              case InternalSubscribeResponse(response @ Replicator.Changed(`key`)) =>
                val value = response.get(key).value.intValue
                updated(value)
